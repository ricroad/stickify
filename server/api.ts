 ...
 * POST /api/generate
 * 
 * 这是一个纯中转 API，Key 从请求头里获取
 * 用户的 API Key 通过 Authorization 头传递
 */
router.post('/generate', upload.single('image'), async (req: Request, res: Response) => {
  try {
    // 1. 获取 Authorization 头 (用户的 API Key)
    ...
    const imageUri = `data:${file.mimetype};base64,${base64Data}`;

    // 4. 定义我们精心设计的 Prompt (手机直出风 + 漫画手绘)
    const SYSTEM_PROMPT = `
